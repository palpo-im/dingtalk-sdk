...
async fn generated_dynamic_endpoint_renders_path_params() {
    ...
    Mock::given(method("POST"))
        .and(path("/v1.0/calendar/users/u1/calendars/c1/events/e1/attendees"))
        .and(header("x-acs-dingtalk-access-token", "token-123"))
...
async fn generated_endpoint_returns_error_when_path_param_missing() {
    let client = DingTalkClient::builder().build().expect("client should build");
    ...
    match result {
        Err(Error::InvalidParam(message)) => {
            assert!(message.contains("event_id"), "unexpected message: {message}");
        }
